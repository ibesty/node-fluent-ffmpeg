...
var lineBreakRegexp = /\r\n|\r|\n/;
var filterRegexp = /^\s*(?:[T\.]\s*[S\.]\s*(?:[C\.])?\s+)?(\S+)\s+(AA?|VV?|N|\|)->(AA?|VV?|N|\|)\s+(.*)$/;
...
module.exports = function(proto) {
  ...
  /**
   * A callback passed to {@link FfmpegCommand#availableHwaccels}.
   *
   * @callback FfmpegCommand~hwaccelCallback
   * @param {Error|null} err error object or null if no error happened
   * @param {Array<String>} hwaccels array of available hardware acceleration methods
   */

  /**
   * Query ffmpeg for available hardware acceleration methods
   *
   * @method FfmpegCommand#availableHwaccels
   * @category Capabilities
   * @aliases getAvailableHwaccels
   *
   * @param {FfmpegCommand~hwaccelCallback} callback callback function
   */
  proto.availableHwaccels =
  proto.getAvailableHwaccels = function(callback) {
    if ('hwaccels' in cache) {
      return callback(null, cache.hwaccels);
    }

    // 运行 ffmpeg -hwaccels 命令获取硬件加速选项
    this._spawnFfmpeg(['-hwaccels'], { captureStdout: true, stdoutLines: 0 }, function (err, stdoutRing) {
      if (err) {
        return callback(err);
      }
      ...
      lines.forEach(function(line) {
        ...
        if (trimmedLine && 
            !trimmedLine.startsWith('Hardware acceleration methods:') &&
            !trimmedLine.startsWith('ffmpeg version') &&
            !trimmedLine.includes('configuration:') &&
            !trimmedLine.includes('built with') &&
            !trimmedLine.includes('libav') &&
            trimmedLine !== 'none') {
          // 添加有效的硬件加速方法
      ...
      callback(null, cache.hwaccels = data);
    });
  };
